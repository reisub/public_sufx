defmodule PublicSuffex do
  import PublicSuffex.RulesParser
  ...
  rules_line_stream =
    if Application.compile_env(:public_suffex, :download_data_on_compile, false) do
      @public_suffix_list_url "https://publicsuffix.org/list/public_suffix_list.dat"

      case __MODULE__.RemoteFileFetcher.fetch_remote_file(@public_suffix_list_url) do
        {:ok, data} ->
          IO.puts("PublicSuffex: fetched fresh data file for compilation.")
          String.split(data, "\n")

        {:error, error} ->
          raise """
          PublicSuffex: failed to fetch fresh data file for compilation:
          #{inspect(error)}
